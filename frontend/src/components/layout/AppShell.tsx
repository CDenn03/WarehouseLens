...
const tenantNavItems: NavItem[] = [
  ...
  },
  {
    href: "/procurement",
    label: "Procurement",
    requiredPermission: "procurement.order.create",
    icon: (
      <svg className={iconClass} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true">
  ...
  },
  {
    href: "/outbound",
    label: "Outbound",
    requiredPermission: "outbound.sales_order.create",
    icon: (
      <svg className={iconClass} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true">
  ...
  },
  {
    href: "/copilot",
    label: "Copilot",
    requiredPermission: "agent.invoke",
    icon: (
      <svg className={iconClass} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true">
  ...
  },
  {
    href: "/admin/roles",
    label: "Roles",
    requiredPermission: "iam.role.manage",
    icon: (
      <svg className={iconClass} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true">
...
export interface AppShellProps {
  ...
  /** Permissions from /auth/me — used to filter sidebar items. */
  permissions: string[];
  userName?: string;
  userEmail?: string;
...
export function AppShell({
  ...
  const { items: allItems, subtitle: brandSubtitle } = dashboard
    ? NAV_BY_DASHBOARD[dashboard]
    : { items: [], subtitle: undefined };

  // Filter out items the current user lacks permission for.
  // Items with no requiredPermission are always shown (e.g. platform nav items
  // use separate nav lists that are only shown to the right dashboard kind).
  const permSet = new Set(permissions);
  const items = allItems.filter(
    (item) => !item.requiredPermission || permSet.has(item.requiredPermission),
  );
