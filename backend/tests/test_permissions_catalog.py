...
class TestPermissionCatalogForm:
    ...
    def test_all_permissions_has_no_duplicates(self):
        """ALL_PERMISSIONS should contain exactly 23 permissions.

        20 operational (incl. warehouse.update) + dashboard.platform +
        dashboard.tenant + platform.tenant.manage.  Deliberate tripwire: adding
        a permission should be a conscious act, and a drop in this count means
        a module's PERMISSIONS dict silently lost an entry.
        """
        assert len(ALL_PERMISSIONS) == 23, f"Expected 23 permissions, got {len(ALL_PERMISSIONS)}"
