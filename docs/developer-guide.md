...
The planner and all six tools are implemented — for the architectural decisions, per-tool query specs, and what's still outstanding (an `eval/run_eval.py` run against a real `LLM_API_KEY`, not yet possible in every environment), see [agent-core-spec.md](./agent-core-spec.md).
...
For the end-to-end user flows that stitch the schema, API surface, and RBAC model together — procurement, external fulfillment, internal transfers, manual adjustments, warehouse/access admin, and dashboard reporting — see [journeys.md](./journeys.md). Its open questions are resolved in §13 items 5-10 above, including one that turned out to be a cross-tenant data leak rather than just an ambiguity.
...
The decisions below resolve [journeys.md](./journeys.md)'s open questions, once the journeys were walked end-to-end against the actual implementation rather than the schema/API sections alone.

5. **Which step writes the `issue`/`transfer_out` transaction?** Resolved: `ship()`, not pick-list completion or delivery. `complete_pick_list` only moves quantity into `warehouse_stock.quantity_reserved`; `ship()` is what decrements `quantity_on_hand` and writes the `inventory_transactions` row (`issue` for external fulfillment, `transfer_out` for internal transfers, chosen by whether the outbound request has a `destination_warehouse_id`). `deliver()` only ever writes `transfer_in`, at the destination, for internal transfers — it never touches the source. This was already the implemented behavior; this entry just records it so it doesn't have to be re-derived from the code again.
6. **Warehouse deactivate/edit and assignment revoke.** Two separate gaps, one real: `PATCH /warehouses/{id}` did not exist (no way to edit a warehouse's name/address or flip `is_active` — the column was defined but never mutated). It now exists, gated by a new `warehouse.update` permission granted to `admin` and `tenant_admin`. Assignment *revoke*, by contrast, already existed — just not where a reader would look for it: `DELETE /iam/users/{user_id}/warehouses/{warehouse_id}` (in the IAM router, alongside role revoke), not `DELETE /warehouses/{id}/assignments/{user_id}`. No further work needed there beyond noting the actual path.
7. **Scoped dashboard/list rollup semantics — and a cross-tenant leak.** Resolved, and it was more than an ambiguity: `scope_filter_warehouse_ids` returned `None` for any `warehouse.global` holder to mean "no filter," and several call sites (`dashboard_service`, `procurement_service.list_purchase_orders`, `outbound_service.list_outbound_requests`, `inventory_service.list_transactions`/`product_stock_breakdown`) took that literally — omitting `warehouse_id` for an Admin/Auditor/Procurement-Officer-class caller rolled up **every warehouse in the database, across every tenant**, not just the caller's tenant. Fixed by making `scope_filter_warehouse_ids` always return a concrete, tenant-scoped `set[UUID]` — for `warehouse.global` holders, every warehouse `id` in `user.tenant_id`, never `None`. Every consumer's `if visible is not None` branch collapsed to an unconditional `.in_(visible)` filter as a result. "Omit `warehouse_id`" now means "roll up across my tenant" for global roles and "roll up across my assigned warehouses" for scoped roles, uniformly, by construction rather than by each call site remembering to check.
8. **Is Procurement Officer warehouse-scoped?** Resolved: yes, same as Warehouse Manager. The role previously held `warehouse.global` (bypassing scoping entirely, contradicting decision 3 above), reportedly so procurement could be actioned across warehouses without per-warehouse setup. That permission is now removed — a Procurement Officer must have a `user_warehouse_assignments` row for a warehouse before creating or receiving a PO against it, and their dashboard rollups cover only their assigned warehouses. `POST /suppliers` stays unscoped (suppliers aren't warehouse-scoped resources).
9. **Manual adjustment auditability.** `inventory_transactions.reason` (free text) is now populated on `POST /inventory/transactions`, and is required (non-blank) specifically when `type='adjustment'` — that's the one transaction type with no PO/shipment to point `reference_id` at, so it was the one case where "why" had no audit trail at all. `created_by` (already a column, previously never written) is now set from the caller's Keycloak `sub` on every manual transaction. Receipt/issue/transfer transactions generated by the procurement/outbound flows are unaffected — this only applies to the manual-adjustment endpoint.
10. **Role restrictions on `POST /sales-orders` and `POST /inventory/transactions`.** Already resolved in the implementation, contrary to what earlier journey notes assumed: `POST /sales-orders` requires `outbound.sales_order.create` (Admin, Warehouse Manager); `POST /inventory/transactions` requires `inventory.write` (Admin, Warehouse Manager). Recorded here so it doesn't get re-flagged as open.
11. **Deactivated-warehouse transfers, and destination-side transfer visibility.** Two follow-ups from walking Journey 3 once `PATCH /warehouses/{id}` made `is_active=false` reachable:
    - `create_internal_transfer` now rejects (409) if either the source or destination warehouse is inactive — matches the journey's stated precondition ("both warehouses ... are active") that had no enforcement before.
    - Resolved in favor of visibility: a Warehouse Manager assigned only to the destination warehouse of an internal transfer can now **see** it (`GET /outbound-requests/{id}` and the list endpoint) before delivery, read-only. New helper `enforce_any_warehouse_scope` (`app/core/security.py`) passes if the caller is assigned to *either* warehouse on the request, unlike `enforce_warehouse_scope`'s single-warehouse check. Write actions — pick-list generation, `ship`, `deliver` — are unchanged and stay source-only, consistent with "RBAC: Warehouse Manager scoped to `source_warehouse_id` (creation, picking, shipping)" above. `list_outbound_requests` now matches source-OR-destination against the caller's visible set, so the transfer also shows up in the destination manager's list view, not just by direct-ID lookup.
    - `create_internal_transfer`'s router (`app/api/v1/outbound.py`) only ever validated the *source* warehouse's tenant — `destination_warehouse_id` was a bare caller-supplied UUID with no tenant check of its own, so a transfer could in principle target a warehouse in a different tenant entirely. Fixed by fetching the destination warehouse and running it through `enforce_tenant_scope` too, same as the source.
